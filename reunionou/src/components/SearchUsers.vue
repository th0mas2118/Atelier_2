...
const emit = defineEmits<{
  (e: 'onSearchResult', val: [object]): void
}>()

const mutateSearchResult = (users: [{}]) => emit('onSearchResult', users)
    ...
    <div>
      <header class="flex flex-col mb-4">
        ...
        <div class="relative">
          ...
          <div
            id="loading-spinner"
            class="flex justify-end align-center absolute w-full top-[8px] right-0"
            v-if="loading"
          >
            <svg
              aria-hidden="true"
              class="w-6 h-6 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-cpurple"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
