...
const Comment = async () => {
  ...
  await fetch('http://api.comment.reunionou:49382/messages/', {
    method: 'POST',
    mode: 'cors',
    body: JSON.stringify({
...
onMounted(() => {
  axios
    .get(`http://api.frontoffice.reunionou:49383/messages/642594b5d4df1915ab0a95f2/`)
    .then((response) => {
...
<template>
  ...
  <div class="container mx-auto h-[60vh] flex flex-col">
    <div class="flex-grow overflow-y-scroll px-4">
      <div class="flex justify-end" v-for="channel in state.channels">
